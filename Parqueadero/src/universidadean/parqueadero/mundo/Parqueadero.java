...
public class Parqueadero {
    ...
    public void avanzarHora() {
        ...
        if(horaActual == HORA_CIERRE){
            horaActual = 0;
            abierto = false;
        } else if(horaActual == HORA_INICIAL){
            abierto = true;
        }
     ...
     * El metodo1 deberá retornar la placa del carro que lleva más tiempo en el parqueadero.
     * Si hay varios carros, deberá retornar cualquiera de ellos.
     * @return Respuesta 1.
     */
    public String metodo1() {
        try{

            String placa = puestos[0].darCarro().darPlaca();
            int mayorTiempo = puestos[0].darCarro().darHoraLlegada();
            for (int i = 0; i < puestos.length - 1; i++) {
                if (puestos[i].estaOcupado()) {
                    if (puestos[i].darCarro().darHoraLlegada() <= mayorTiempo) {
                        placa = puestos[i].darCarro().darPlaca();
                    }
                }
            }
            return placa;

        } catch (Exception e){ return "No hay ningún auto en el parqueadero";}
    }

    /**
     * Método de extensión 2.
     * El metodo2 deberá retornar la frase El parqueadero está lleno cuando no haya puestos disponibles en el parqueadero,
     y la frase Hay puestos disponibles si hay al menos un puesto libre en el parqueadero.
     * @return Respuesta 2.
     */
