...
public class FeriaEmpresarial {
     ...
     */
    /*
    El metodo1 se trata de un método que se encargue de saber cuál es la zona con mayor número de expositores.
    Si hay dos o más Zonas iguales reporte cualquiera.
     */
    public String metodo1() {

        int[] zonas = new int[5];
        int zonaMayorExpositores = 0;
        String resultado = "";
        for(Puesto i : puestos){
            if(i.estaOcupado()){
                for(Empresa j : empresas){
                    if(i.darNombreEmpresa().equals(j.darNombre()) && j.darTipo().equals(Empresa.EXPOSITORA)){
                        switch (i.darZona()) {
                            case Puesto.ZONA_NORTE -> zonas[0] += j.darNumeroPersonasAsistentes();
                            case Puesto.ZONA_ORIENTE -> zonas[1] += j.darNumeroPersonasAsistentes();
                            case Puesto.ZONA_SUR -> zonas[2] += j.darNumeroPersonasAsistentes();
                            case Puesto.ZONA_OCCIDENTE -> zonas[3] += j.darNumeroPersonasAsistentes();
                            case Puesto.ZONA_CENTRO -> zonas[4] += j.darNumeroPersonasAsistentes();
                        }
                    }
                }
            }
       ...
       for(int i=4;i>=0;i--){
           if(zonaMayorExpositores < zonas[i]){
               zonaMayorExpositores = zonas[i];
               switch(i){
                   case 0 -> resultado = Puesto.ZONA_NORTE;
                   case 1 -> resultado = Puesto.ZONA_ORIENTE;
                   case 2 -> resultado = Puesto.ZONA_SUR;
                   case 3 -> resultado = Puesto.ZONA_OCCIDENTE;
                   case 4 -> resultado = Puesto.ZONA_CENTRO;
               }
           }
       }

        return resultado+" tiene la mayor cantidad de expositores";
    }
    ...
    /*
    busca verificar si el porcentaje total de expositores actualmente
    es por lo menos el 60% de expositores para tener el punto de equilibrio.
     */
    public String metodo2() {
        float nExpositoras = 0;
        float porcentaje;
        String resultado;
        for(Empresa i : empresas){
            if(i.darTipo().equals(Empresa.EXPOSITORA)){
                ++nExpositoras;
            }
        }
        porcentaje = ((nExpositoras*100)/22);
        resultado = (porcentaje >= 60) ? "El punto de equilibrio se ha cumplido" : "El punto de equilibrio no se ha cumplido";
